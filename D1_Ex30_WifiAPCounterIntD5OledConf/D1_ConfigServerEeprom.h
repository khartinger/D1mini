//_____D1_ConfigServerEeprom.h_____________________20170226_____
// uses: ADDR_SIZE, NAME_SIZE, SSID_SIZE, PASS_SIZE
//       sign_[3], name_[NAME_SIZE+1], ssid_[SSID_SIZE+1],
//                 pass_[PASS_SIZE+1]
//       IPAddress ip, gateway, subnet
#ifndef D1_CONFIGSERVEREEPROM_H
#define D1_CONFIGSERVEREEPROM_H
#include "EEPROM.h"
...
extern void setup_webserver();    //name of webserver function
...
void setup_config_server()
{
// uint8_t data_[EEPROM_SIZE];      // tempory config data
...
void handle_config_server() 
{
 ...
 {
  String s2="Unknown config mode "+String(config)+".";
 ...
 if(!server.argName(15).equals("ex"))
 {
 ...
 if(server.arg(15).indexOf("Abbrech")>=0)
 {
  config=0;                      // config finished
  Serial.println("Konfiguration wurde abgebrochen.");
  handleRoot();
  return;
 }  
 //-----wrong parameter for "ex"--------------------------------
 if(server.arg(15).indexOf("bernehmen")<1)
 {
...
//  Serial.println("check ok");
 ...
 if(!server.argName(12).equals("n")) return -120;
 s1=server.arg(12);
 len_=s1.length();
 if(len_>NAME_SIZE) return -121;
 for(i=0; i<len_; i++) tmp_[i0++]=s1[i];
 for(i=len_; i<NAME_SIZE; i++) tmp_[i0++]=0;
 //-----check ssid----------------------------------------------
 if(!server.argName(13).equals("s")) return -130;
 s1=server.arg(13);
 len_=s1.length();
 ...
 //-----check password------------------------------------------
 if(!server.argName(14).equals("p")) return -140;
 s1=server.arg(14);
 len_=s1.length();
